...
pub(crate) fn parse_domain() -> Option<DomainModel> {
    ...
    let score_type = find_score_type(&all_src, &solution_type)
        .unwrap_or_else(|| "HardSoftScore".to_string());
    ...
    let entities: Vec<EntityInfo> = entities_raw
        ...
        .map(|(field_name, item_type)| {
            ...
            EntityInfo { field_name, item_type, planning_vars }
        })
        .collect();

    let facts: Vec<FactInfo> = facts_raw
        .into_iter()
        .map(|(field_name, item_type)| FactInfo { field_name, item_type })
        .collect();
    ...
    Some(DomainModel { solution_type, score_type, entities, facts })
}
        ...
        if t.contains(&format!("#[{}]", attr)) || t.contains(&format!("#[{}(", attr)) {
            ...
            for j in (i + 1)..lines.len().min(i + 4) {
                ...
                if next.starts_with("pub struct ") || next.starts_with("struct ") {
                    let after = next.trim_start_matches("pub ").trim_start_matches("struct ");
                    let name: String = after.chars().take_while(|c| c.is_alphanumeric() || *c == '_').collect();
                    if !name.is_empty() {
                        return Some(name);
                    }
...
fn find_collections(src: &str, solution_type: &str) -> (Vec<(String, String)>, Vec<(String, String)>) {
    let lines: Vec<&str> = src.lines().collect();
    let mut in_solution_struct = false;
    let mut brace_depth = 0i32;
    ...
    for line in &lines {
        ...
        if t.contains("#[planning_entity_collection]") || t.contains("#[planning_entity_collection(") {
            next_annotation = Some("entity");
        } else if t.contains("#[problem_fact_collection]") || t.contains("#[problem_fact_collection(") {
...
pub(crate) fn parse_vec_field(line: &str) -> Option<(String, String)> {
    ...
    if let Some(colon) = t.find(':') {
        ...
        if type_part.starts_with("&[") || type_part.starts_with('[') {
            let inner = type_part.trim_start_matches('&').trim_start_matches('[').trim_end_matches(']');
            if !inner.is_empty() {
