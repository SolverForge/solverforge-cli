...
pub(crate) fn resolve_pattern_and_hardness(
    ...
    let pattern_implies_soft = matches!(explicit_pattern, Some(Pattern::Balance) | Some(Pattern::Reward));
...
fn run_wizard(soft_flag: bool, domain: &Option<DomainModel>) -> Result<(Pattern, bool), String> {
    ...
    if let Some(d) = domain {
        ...
        for e in &d.entities {
            ...
            println!("    Entities:  {} ({}){}", e.field_name.bright_cyan(), e.item_type, var_info);
        }
        for f in &d.facts {
            println!("    Facts:     {} ({})", f.field_name.bright_cyan(), f.item_type);
        }
        println!();
    }

    // Pattern selection
    let has_join = domain.as_ref().map(|d| !d.facts.is_empty()).unwrap_or(false);

    let mut pattern_options: Vec<(&str, Pattern)> = vec![
        ("Penalize matching entities        (e.g. unassigned, invalid state)", Pattern::Unary),
        ("Penalize conflicting pairs        (e.g. overlapping shifts, double-booking)", Pattern::Pair),
    ];
    if has_join {
        pattern_options.push(("Penalize entity-fact mismatch     (e.g. missing skill, wrong location)", Pattern::Join));
    }
    pattern_options.push(("Balance assignments                (e.g. fair workload distribution)", Pattern::Balance));
    pattern_options.push(("Reward matching entities           (e.g. desired day, preferred shift)", Pattern::Reward));
