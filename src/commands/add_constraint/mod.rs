...
use domain::parse_domain as parse_domain_inner;
use mod_rewriter::{extract_types, rewrite_mod};
use skeleton::generate_skeleton;
use utils::snake_to_title;
use wizard::resolve_pattern_and_hardness;

// ─── Public entry point ───────────────────────────────────────────────────────

/// Runs `solverforge add constraint <name> [pattern flags] [--hard|--soft]`.
pub fn run(
    name: &str,
    soft: bool,
    unary: bool,
    ...
    if !constraints_dir.exists() {
        return Err(
            "not a SolverForge project directory (src/constraints/ not found)".to_string(),
        );
    }
    if !mod_path.exists() {
    ...
    let (pattern, is_soft) = resolve_pattern_and_hardness(
        soft, unary, pair, join, balance, reward, &domain,
    )?;
