...
pub(crate) fn update_domain_mod(mod_name: &str, pascal: &str) -> Result<(), String> {
    ...
    fs::write(mod_path, new_src)
        .map_err(|e| format!("failed to write src/domain/mod.rs: {}", e))
}
...
pub(crate) fn wire_collection_into_solution(
    ...
    let field_block = format!("    #[{}]\n    pub {}: Vec<{}>,", annotation, plural, pascal);
...
pub(crate) fn add_import(src: &str, import: &str) -> String {
    ...
    let last_use = lines.iter().rposition(|l| l.trim_start().starts_with("use "));
    let insert_at = last_use.map(|i| i + 1).unwrap_or(0);
    lines.insert(insert_at, import);
    lines.join("\n") + "\n"
}

pub(crate) fn insert_struct_field(src: &str, _solution_type: &str, field_block: &str) -> Result<String, String> {
    ...
    // Find last `}` in file as fallback insertion point (before closing brace of struct)
    // Look for the pattern `}` on its own line preceded by a field
    let lines: Vec<&str> = src.lines().collect();
    // Find the closing `}` of the solution struct by finding `}` with no indentation after fields
    for (i, line) in lines.iter().enumerate().rev() {
        if line.trim() == "}" {
            // Check if previous non-empty line looks like a struct field
            let prev = lines[..i].iter().rev().find(|l| !l.trim().is_empty());
            if prev.map(|l| l.trim_start().starts_with("pub ") || l.trim() == "}").unwrap_or(false) {
...
fn add_constructor_param(src: &str, _solution_type: &str, pascal: &str, plural: &str) -> String {
    ...
    let src = if src.contains("pub fn new()") {
        src.replacen("pub fn new()", &format!("pub fn new({}: Vec<{}>)", plural, pascal), 1)
        ...
        if let Some(start) = src.find(marker) {
            ...
            if let Some(end) = after.find(')') {
                ...
                format!("{}{}{}", &src[..replace_from], new_params, &src[replace_to..])
            } else {
                src.to_string()
            }
        } else {
...
pub(crate) fn inject_planning_variable(src: &str, entity: &str, field: &str) -> Result<String, String> {
    let field_block = format!(
    ...
    let src = add_self_init_field(&src, &format!("{}: None", field).replace(": None", ""));
...
pub(crate) fn replace_score_type(src: &str, old_score: &str, new_score: &str) -> Result<String, String> {
    if !src.contains(old_score) {
        return Err(format!("score type '{}' not found in solution file", old_score));
    }
