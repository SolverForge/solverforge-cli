...
/// Shift with embedded employee (denormalized for the API).
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ShiftDto {
    pub id: String,
...
/// Full schedule for API request/response.
...
impl ScheduleDto {
    pub fn from_schedule(schedule: &EmployeeSchedule, status: Option<SolverStatus>) -> Self {
        let employees: Vec<EmployeeDto> =
            schedule.employees.iter().map(EmployeeDto::from).collect();
